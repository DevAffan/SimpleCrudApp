<x-app-layout>
    ...
    <div class="container mx-auto mt-10">
        ...
        <!-- Create Contact Form -->
        <form action="{{ route('contact.update', $contact) }}" method="POST" class="bg-white p-6 rounded-lg shadow-md">
            @csrf
            @method('PUT')
            <div class="mb-4">
                <label for="name" class="block text-gray-700 font-semibold">Name</label>
                <input type="text" id="name" name="name" value="{{$contact->name}}" class="w-full border border-gray-300 p-2 rounded-lg" required>
            </div>
